...
contract ArtMarketplace is ArtToken{
  ...
  event itemWithdrawn(uint256 id, address seller); // New event for item withdrawal
  ...
  modifier IsNotBidding() {
    require(itemsForSale.length == 0, "Bidding is not supported");
    _;
  }

  modifier CanWithdrawItem(uint256 id) {
    require(msg.sender == itemsForSale[id].seller, "You are not the seller of this item");
    require(!itemsForSale[id].isSold, "Item is already sold");
    _;
  }
  ...
  function withdrawItem(uint256 id)
    ItemExists(id)
    CanWithdrawItem(id)
    external
  {
    ...
    emit itemWithdrawn(id, msg.sender);
  }
